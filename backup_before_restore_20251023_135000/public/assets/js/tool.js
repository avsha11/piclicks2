...
let isCapturingCanvas = false; // Flag to prevent MutationObserver infinite loop during canvas capture
    ...
    if (sortableElement) {
        sortableObj = Sortable.create(sortableElement, {
            ...
            onEnd: async function (evt) {
                ...
                placeTiles();
                // Update mask after tiles are rearranged
                setTimeout(() => window.updateTextMask(), 100);
            },
        });
    } else {
...
document.querySelector("#change-size-btn").addEventListener("click", () => {
    ...
    setTimeout(async () => {
        await calcGridDimension()
        placeTiles(); // Recalculate grid layout
        // Update mask after tiles are resized
        setTimeout(() => window.updateTextMask(), 100);
    }, 500);
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function addTextOverlay() {
        ...
        // Get grid and container references
        const grid = document.getElementById('preview-grid');
        const container = grid ? grid.closest('.tool-inner') || grid.parentElement : null;
        ...
        // Ensure text layer exists and move text to it
        ...
        // Position text layer to align with grid
        const gridRect = grid.getBoundingClientRect();
        const containerRect = container.getBoundingClientRect();
        textLayer.style.left = `${gridRect.left - containerRect.left}px`;
        textLayer.style.top = `${gridRect.top - containerRect.top}px`;
        textLayer.style.width = `${gridRect.width}px`;
        textLayer.style.height = `${gridRect.height}px`;
        ...
        // Don't append to toolmiddle since text is already in textLayer
        // toolmiddle.appendChild(textOverlay);
        
        makeDraggableAndRotatable(textOverlay, rotateHandle);
        
        // Apply mask first, then select after a delay
        setTimeout(() => {
            if (window.updateTextMask) {
                window.updateTextMask();
            }
        }, 200);
        
        // Select text after mask is applied
        setTimeout(() => {
    ...
    function makeDraggableAndRotatable(element, rotateHandle) {
        ...
        function rotateMove(e) {
            ...
            // Mask will be updated in stopAction
        }
        ...
        function stopAction() {
            ...
            // Update mask after text movement/rotation only if not being edited
            if (!window.isTextBeingEdited && window.updateTextMask) {
                setTimeout(() => window.updateTextMask(), 50);
            }
        }
        // Event Listeners
    ...
    function selectOverlay(overlay) {
        ...
        // Set editing flag and show text entirely when selected (remove mask)
        window.isTextBeingEdited = true;
        showTextEntirely();
    }
    $txtModal.on('shown.bs.modal', function () {
        ...
        // Clear editing flag and hide text with mask when deselected
        window.isTextBeingEdited = false;
        setTimeout(() => {
            hideTextWithMask();
        }, 100);
    ...
    function restoreTextOverlays() {
        ...
        storedTextOverlays.forEach(overlay => {
            ...
            if (textLayer) {
                textLayer.appendChild(textOverlay);
            } else if (window.updateTextMask) {
                // If text layer doesn't exist yet, use the update function
                setTimeout(() => window.updateTextMask(), 100);
            }
            
            // Update mask after text restoration
            if (window.updateTextMask) {
                setTimeout(() => window.updateTextMask(), 50);
            }
...
async function saveCollage(type) {
    ...
    requestIdleCallback(() => {
        ...
        setTimeout(() => {
            // Set flag to prevent MutationObserver from triggering during canvas capture
            window.isCapturingCanvas = true;
            ...
            // Store original tile positions to remove gaps for preview
            ...
            $grid.find(".image-div").each(function(index) {
                ...
                const newLeft = (col * actualWidth) + 'px';
                const newTop = (row * actualHeight) + 'px';
                
                console.log(`Tile ${index}: ${originalLeft},${originalTop} -> ${newLeft},${newTop} (col:${col}, row:${row})`);
            ...
            // Also adjust grid size to account for removed margins
            const columns = parseInt(document.getElementById("grid_columns").value);
            const rows = parseInt(document.getElementById("grid_rows").value);
            const originalGridWidth = $previewGrid.css('width');
            const originalGridHeight = $previewGrid.css('height');
            $previewGrid.css({
            ...
            // Note: CSS filters won't be captured, but server-side filter application in CollageController.php handles this
            console.log('=== STARTING CANVAS CAPTURE ===');
            console.log('Grid element:', $gridMiddle[0]);
            console.log('Grid dimensions:', $gridMiddle[0].offsetWidth, 'x', $gridMiddle[0].offsetHeight);
            ...
            html2canvas($gridMiddle[0], {
                ...
                window.isCapturingCanvas = false;
                ...
                // Restore background, grid size, and tile positions even on error
                const $previewGrid = $('#preview-grid');
                $previewGrid.css({
                    'background': originalBackground,
                    'background-color': originalBackgroundColor,
...
function addResizeDrag(tile, handle) {
    ...
    handle.onmousedown = function (e) {
        ...
        document.onmouseup = function () {
            ...
            calcGridDimension();
            placeTiles();
            // Update mask after tile resize
            setTimeout(() => window.updateTextMask(), 100);
        };
    };
}

// Simple visual-based text clipping using CSS mask
(() => {
    const grid = document.getElementById('preview-grid');
    if (!grid) return;
    ...
    // Create text layer that covers the entire grid
    let textLayer = document.getElementById('text-layer');
    if (!textLayer) {
        textLayer = document.createElement('div');
        textLayer.id = 'text-layer';
    ...
    // Move text overlays to text layer
    function moveTextsToLayer() {
        ...
        if (textOverlays.length === 0) {
            // Check if text is in the grid directly
            const gridTexts = grid.querySelectorAll('.text-overlay');
            
            // Check if text is in the container
            const containerTexts = container.querySelectorAll('.text-overlay');
            
            // Check if text is anywhere in the document
            const allTexts = document.querySelectorAll('.text-overlay');
            
            if (allTexts.length > 0) {
                allTexts.forEach((overlay, index) => {
                    textLayer.appendChild(overlay);
                });
            }
        } else {
            textOverlays.forEach((overlay, index) => {
                if (overlay.parentElement !== textLayer) {
                    textLayer.appendChild(overlay);
                }
            });
        }
    ...
    // Create visual mask using ALL tiles, excluding empty ones
    function createVisualMask() {
        if (!textLayer) return;
        ...
        // Get grid dimensions for accurate positioning
        const gridRect = grid.getBoundingClientRect();
        const containerRect = container.getBoundingClientRect();
        
        // Ensure text layer is positioned correctly before calculating mask
        const textLayerLeft = gridRect.left - containerRect.left;
        const textLayerTop = gridRect.top - containerRect.top;
        
        textLayer.style.left = `${textLayerLeft}px`;
        textLayer.style.top = `${textLayerTop}px`;
        textLayer.style.width = `${gridRect.width}px`;
        textLayer.style.height = `${gridRect.height}px`;
        
        // Get CSS-defined tile dimensions
        const computedStyle = window.getComputedStyle(document.documentElement);
        const cssTileWidth = parseFloat(computedStyle.getPropertyValue('--image-div-width')) || 91;
        const cssTileHeight = parseFloat(computedStyle.getPropertyValue('--image-div-height')) || 80;
        const cssMargin = 2; // From CSS: margin: 2px;
        
        // Calculate scale factor from actual vs CSS dimensions
        const firstTile = grid.querySelector('.image-div');
        let scaleFactor = 1;
        if (firstTile) {
            const actualRect = firstTile.getBoundingClientRect();
            scaleFactor = actualRect.width / cssTileWidth;
        }
        
        // VISUAL DETECTION: Use the same logic as showMaskDebug() which is working correctly
        const occupiedTiles = [];
        let occupiedCount = 0;
        
        // Get all actual tiles from DOM (same as showMaskDebug)
        const allTiles = grid.querySelectorAll('.image-div');
        console.log(`Found ${allTiles.length} actual tiles in DOM`);
        
        // Process each actual tile (same logic as showMaskDebug)
        allTiles.forEach((tile, index) => {
            const tileRect = tile.getBoundingClientRect();
            
            // Check if tile is visually occupied (same logic as showMaskDebug)
            const img = tile.querySelector('img');
            const isOccupied = img && 
            ...
            if (isOccupied) {
                ...
                console.log(`Tile ${index}: occupied at ${x.toFixed(1)}x${y.toFixed(1)} size ${w.toFixed(1)}x${h.toFixed(1)}`);
                
                // Create rounded rectangle polygon for this occupied tile
                const radius = 7; // R8 in editor/preview as per checklist
                const polygon = [
                    `${(x + radius).toFixed(2)}px ${(y).toFixed(2)}px`,
                    `${(x + w - radius).toFixed(2)}px ${(y).toFixed(2)}px`,
                    `${(x + w).toFixed(2)}px ${(y + radius).toFixed(2)}px`,
                    `${(x + w).toFixed(2)}px ${(y + h - radius).toFixed(2)}px`,
                    `${(x + w - radius).toFixed(2)}px ${(y + h).toFixed(2)}px`,
                    `${(x + radius).toFixed(2)}px ${(y + h).toFixed(2)}px`,
                    `${(x).toFixed(2)}px ${(y + h - radius).toFixed(2)}px`,
                    `${(x).toFixed(2)}px ${(y + radius).toFixed(2)}px`
                ].join(', ');
                
                occupiedTiles.push(polygon);
                occupiedCount++;
            }
        });
        
        console.log(`Created ${occupiedCount} holes for visually occupied tiles`);
        
        // Apply clip-path directly to text layer to create holes
        if (occupiedTiles.length > 0) {
            const clipPath = `polygon(${occupiedTiles.join(', ')})`;
            
            // Use only setProperty with important flag
            textLayer.style.setProperty('clip-path', clipPath, 'important');
            textLayer.style.setProperty('-webkit-clip-path', clipPath, 'important');
            
            // Also set it as a CSS custom property for backup
            textLayer.style.setProperty('--mask-clip-path', clipPath);
        } else {
            // Hide text if no occupied tiles
            textLayer.style.setProperty('clip-path', 'polygon(0 0, 0 0, 0 0)', 'important');
            textLayer.style.setProperty('-webkit-clip-path', 'polygon(0 0, 0 0, 0 0)', 'important');
        }
    }
    
    // Show text entirely when selected (remove mask)
    function showTextEntirely() {
        if (textLayer) {
    ...
    // Hide text when not selected (apply mask)
    function hideTextWithMask() {
        // Only apply mask if text is not being actively edited
        if (!window.isTextBeingEdited) {
            createVisualMask();
        }
    }
    
    // Update mask when grid changes
    function updateTextMask() {
        moveTextsToLayer();
        
        // Wait for text to be moved before applying mask
        setTimeout(() => {
            createVisualMask();
        }, 10);
    }
    ...
    // Simple approach: always apply mask, no event interference
    // The mask will be applied and stay applied
    
    // Remove the problematic interval that was causing infinite loops
    // The mask will be applied once and stay applied
    
    // Observe grid changes
    const observer = new MutationObserver(() => {
        if (!window.isCapturingCanvas) {
            updateTextMask();
        }
    });
    ...
    window.updateTextMask = updateTextMask;
    window.showTextEntirely = showTextEntirely;
    window.hideTextWithMask = hideTextWithMask;
    window.debugMask = () => {
        console.log('Text layer children:', textLayer.children.length);
        console.log('Text layer clip-path:', textLayer.style.clipPath);
        console.log('Text layer webkit clip-path:', textLayer.style.webkitClipPath);
    };
    
    // Test function to force text movement and mask application
    window.testTextMask = function() {
        console.log('=== TESTING TEXT MASK ===');
        
        // Force move all text overlays
        moveTextsToLayer();
        
        // Force mask creation
        createVisualMask();
        
        // Check if text overlays are actually in the text layer
        const textOverlays = document.querySelectorAll('.text-overlay');
        console.log('All text overlays in DOM:', textOverlays.length);
        console.log('Text overlays in text layer:', textLayer.querySelectorAll('.text-overlay').length);
        
        textOverlays.forEach((overlay, index) => {
            console.log(`Text overlay ${index} parent:`, overlay.parentElement.id || overlay.parentElement.tagName);
        });
        
        // Check computed styles
        const computedStyle = window.getComputedStyle(textLayer);
        console.log('Computed clip-path:', computedStyle.clipPath);
        console.log('Computed webkit clip-path:', computedStyle.webkitClipPath);
    };
    
    // Test function to force mask application without event interference
    window.forceMask = function() {
        console.log('=== FORCING MASK APPLICATION ===');
        moveTextsToLayer();
        createVisualMask();
        
        // Force the mask to stay applied
        setTimeout(() => {
            console.log('Final check - Text layer clip-path:', textLayer.style.clipPath);
            console.log('Final check - Text layer webkit clip-path:', textLayer.style.webkitClipPath);
            
            const computedStyle = window.getComputedStyle(textLayer);
            console.log('Final check - Computed clip-path:', computedStyle.clipPath);
            console.log('Final check - Computed webkit clip-path:', computedStyle.webkitClipPath);
        }, 100);
        
        console.log('Mask forced - should now be visible');
    };
    
    // Nuclear option: completely override any CSS interference
    window.nuclearMask = function() {
        console.log('=== NUCLEAR MASK APPLICATION ===');
        
        // Remove any existing CSS that might interfere
        const style = document.createElement('style');
        style.id = 'nuclear-mask-style';
        style.textContent = `
            #text-layer {
                clip-path: var(--mask-clip-path) !important;
                -webkit-clip-path: var(--mask-clip-path) !important;
            }
        `;
        
        // Remove old style if it exists
        const oldStyle = document.getElementById('nuclear-mask-style');
        ...
        console.log('Nuclear mask applied - should be bulletproof now');
    };
    
    // Ultimate fix: Force clip-path to stay applied
    window.forceClipPath = function() {
        console.log('=== FORCING CLIP-PATH TO STAY ===');
        
        // First, get the current mask
        moveTextsToLayer();
        createVisualMask();
        
        // Get the clip-path that was just created
        const clipPath = textLayer.style.getPropertyValue('--mask-clip-path');
        console.log('Clip-path to force:', clipPath);
        
        if (!clipPath) {
            console.log('No clip-path found, creating one...');
            return;
        }
        
        // Create a style that forces the clip-path to stay
        const style = document.createElement('style');
        style.id = 'force-clip-path-style';
        style.textContent = `
            #text-layer {
        ...
        // Remove old style if it exists
        const oldStyle = document.getElementById('force-clip-path-style');
        if (oldStyle) oldStyle.remove();
        
        document.head.appendChild(style);
        
        // Force reapply every 50ms to prevent clearing
        const forceInterval = setInterval(() => {
            if (textLayer) {
                textLayer.style.setProperty('clip-path', clipPath, 'important');
                textLayer.style.setProperty('-webkit-clip-path', clipPath, 'important');
                
                // Also force it on all text overlays
                const textOverlays = textLayer.querySelectorAll('.text-overlay');
                textOverlays.forEach(overlay => {
                    overlay.style.setProperty('clip-path', 'inherit', 'important');
                    overlay.style.setProperty('-webkit-clip-path', 'inherit', 'important');
                });
            }
        }, 50);
        
        // Store interval for cleanup
        window.forceClipPathInterval = forceInterval;
        
        console.log('Force clip-path applied - will reapply every 50ms');
    };
    
    // Stop forcing clip-path
    window.stopForceClipPath = function() {
        if (window.forceClipPathInterval) {
            clearInterval(window.forceClipPathInterval);
            window.forceClipPathInterval = null;
            console.log('Stopped forcing clip-path');
        }
    };
    
    // Test function to make text visible without mask
    window.showTextWithoutMask = function() {
        console.log('=== SHOWING TEXT WITHOUT MASK ===');
        
        if (textLayer) {
            textLayer.style.setProperty('clip-path', 'none', 'important');
            textLayer.style.setProperty('-webkit-clip-path', 'none', 'important');
            
            // Also remove from individual text overlays
            const textOverlays = textLayer.querySelectorAll('.text-overlay');
            textOverlays.forEach(overlay => {
                overlay.style.setProperty('clip-path', 'none', 'important');
                overlay.style.setProperty('-webkit-clip-path', 'none', 'important');
            });
            
            console.log('Text should now be visible without mask');
            
            // Stop any running intervals to prevent infinite loops
            if (window.forceClipPathInterval) {
                clearInterval(window.forceClipPathInterval);
                window.forceClipPathInterval = null;
            }
    ...
    // Test function to check text layer visibility
    window.checkTextVisibility = function() {
        console.log('=== CHECKING TEXT VISIBILITY ===');
        
        if (textLayer) {
            const computedStyle = window.getComputedStyle(textLayer);
            console.log('Text layer display:', computedStyle.display);
            console.log('Text layer visibility:', computedStyle.visibility);
            console.log('Text layer opacity:', computedStyle.opacity);
            console.log('Text layer clip-path:', computedStyle.clipPath);
            console.log('Text layer webkit clip-path:', computedStyle.webkitClipPath);
        }
    };
    
    // Debug function to show mask alignment
    window.debugMaskAlignment = function() {
        console.log('=== DEBUGGING MASK ALIGNMENT ===');
        
        const grid = document.getElementById('preview-grid');
        const container = grid ? grid.closest('.tool-inner') || grid.parentElement : null;
        
        if (!grid || !container) {
            console.log('Grid or container not found');
            return;
        }
        ...
        console.log('Text layer rect:', textLayer ? textLayer.getBoundingClientRect() : 'No text layer');
        
        // Show first few occupied tiles
        const allTiles = grid.querySelectorAll('.image-div');
        let count = 0;
        allTiles.forEach(tile => {
            if (count >= 3) return; // Only show first 3
            ...
            if (!isEmpty && imgSrc.trim() !== '') {
                ...
                console.log(`Tile ${count}:`, {
                    tileRect: tileRect,
                    relativeToGrid: { x, y, width: tileRect.width, height: tileRect.height },
    ...
    // Visual debugging function to overlay actual tile areas
    window.showMaskDebug = function() {
        console.log('=== SHOWING TILE DEBUG OVERLAY ===');
        
        // Remove existing debug overlay
        const existing = document.getElementById('mask-debug-overlay');
        ...
        debugOverlay.id = 'mask-debug-overlay';
        debugOverlay.style.cssText = `
            position: absolute;
            left: ${gridRect.left - containerRect.left}px;
            top: ${gridRect.top - containerRect.top}px;
        ...
        // Get CSS-defined tile dimensions (not scaled)
        const computedStyle = window.getComputedStyle(document.documentElement);
        const cssTileWidth = parseFloat(computedStyle.getPropertyValue('--image-div-width')) || 91;
        const cssTileHeight = parseFloat(computedStyle.getPropertyValue('--image-div-height')) || 80;
        ...
        allTiles.forEach((tile, index) => {
            ...
            const imgSrc = img.src || img.getAttribute('src') || '';
            const isEmpty = imgSrc.includes('grey-back.png') || imgSrc.includes('grey-back') || imgSrc.includes('placeholder') || imgSrc.trim() === '';
            ...
            if (!isEmpty && imgSrc.trim() !== '') {
                ...
                // Use CSS-defined dimensions scaled to match actual grid
                const w = cssTileWidth * scaleFactor;
                const h = cssTileHeight * scaleFactor;
                ...
                debugRect.style.cssText = `
                    ...
                    border-radius: 7px;
                `;
                debugOverlay.appendChild(debugRect);
                occupiedCount++;
            }
        });
        
        console.log(`Red rectangles show ${occupiedCount} individual occupied tiles`);
    };
    
    // Show the actual CSS clip-path mask visually
    window.showClipPathMask = function() {
        console.log('=== SHOWING CSS CLIP-PATH MASK VISUALLY ===');
        
        // Remove existing debug overlay
        const existing = document.getElementById('clip-path-debug');
        if (existing) existing.remove();
        ...
        // Create a visual representation of the clip-path
        const clipPathDebug = document.createElement('div');
        clipPathDebug.id = 'clip-path-debug';
        clipPathDebug.style.cssText = `
            position: absolute;
            left: ${gridRect.left - containerRect.left}px;
            top: ${gridRect.top - containerRect.top}px;
            width: ${gridRect.width}px;
        ...
        `;
        container.appendChild(clipPathDebug);
        
        // Get the current clip-path from text layer
        if (textLayer) {
            const computedStyle = window.getComputedStyle(textLayer);
            const clipPath = computedStyle.clipPath || textLayer.style.clipPath;
            console.log('Current clip-path:', clipPath);
            
            // Apply the same clip-path to our debug element
            clipPathDebug.style.clipPath = clipPath;
            clipPathDebug.style.webkitClipPath = clipPath;
            
            console.log('Green overlay shows what the clip-path mask allows to be visible');
        }
    };
    
    // Show tile mask debug (red rectangles only)
    window.showFullMaskDebug = function() {
        console.log('=== SHOWING TILE MASK DEBUG ===');
        
        // Show occupied tiles in red
        showMaskDebug();
        
        console.log('RED = Occupied tiles (where text should be visible)');
        console.log('Red rectangles should align perfectly with photo tiles on the grid');
    };
    
    // Show the exact polygon coordinates being used
    window.showPolygonCoords = function() {
        console.log('=== SHOWING POLYGON COORDINATES ===');
        ...
        allTiles.forEach((tile, index) => {
            ...
            if (!isEmpty && imgSrc.trim() !== '') {
                ...
                console.log(`Tile ${occupiedCount}: ${x.toFixed(1)}x${y.toFixed(1)} ${w.toFixed(1)}x${h.toFixed(1)}`);
                occupiedCount++;
            }
        });
        
        console.log(`Total occupied tiles: ${occupiedCount}`);
    };
    
    // Remove debug overlay
    window.hideMaskDebug = function() {
        const existing = document.getElementById('mask-debug-overlay');
        if (existing) existing.remove();
    ...
    window.clearAllDebug = function() {
        const overlays = ['mask-debug-overlay', 'clip-path-debug'];
        overlays.forEach(id => {
            const element = document.getElementById(id);
            if (element) element.remove();
        });
        console.log('All debug overlays cleared');
    };
    
    // Force refresh mask with detailed debugging
    window.refreshMask = function() {
        console.log('=== FORCING MASK REFRESH ===');
        
        // Clear existing debug overlays
        clearAllDebug();
        
        // Force mask recreation
        if (window.createVisualMask) {
            createVisualMask();
        }
        ...
        console.log('Mask refreshed - check console for detailed coordinates');
    };
    ...
    window.checkTextVisibility = function() {
        ...
        if (textLayer) {
            ...
            const textOverlays = textLayer.querySelectorAll('.text-overlay');
            console.log('Text overlays in text layer:', textOverlays.length);
            
            textOverlays.forEach((overlay, index) => {
                const overlayStyle = window.getComputedStyle(overlay);
                console.log(`Text overlay ${index}:`, {
                    display: overlayStyle.display,
                    visibility: overlayStyle.visibility,
                    opacity: overlayStyle.opacity,
                    clipPath: overlayStyle.clipPath,
                    webkitClipPath: overlayStyle.webkitClipPath
                });
            });
        }
    };
    
    // Simple function to apply mask without interference
    window.applySimpleMask = function() {
        console.log('=== APPLYING SIMPLE MASK ===');
    ...
    window.rebuildMask = function() {
        console.log('=== REBUILDING MASK SYSTEM ===');
        ...
        // Move text overlays to new layer
        moveTextsToLayer();
        
        // Apply mask
        createVisualMask();
        
        console.log('Mask system rebuilt from scratch');
    };
    
    // Force add text to text layer
    window.forceAddText = function() {
        console.log('=== FORCING TEXT ADDITION ===');
        
        // Find all text overlays in the entire document
        const allTexts = document.querySelectorAll('.text-overlay');
        console.log(`Found ${allTexts.length} text overlays in document`);
        
        if (allTexts.length > 0) {
            allTexts.forEach((overlay, index) => {
                if (overlay.parentElement !== textLayer) {
                    textLayer.appendChild(overlay);
                    console.log(`Force moved text overlay ${index} to text layer`);
                }
            });
            
            // Apply mask after moving text
            createVisualMask();
        } else {
            console.log('No text overlays found to add');
        }
    };
    
    // Simple test to check if mask is working
    window.checkMask = function() {
        console.log('=== CHECKING MASK STATUS ===');
        console.log('Text layer exists:', !!textLayer);
        console.log('Text layer children:', textLayer ? textLayer.children.length : 0);
        console.log('Text layer style clip-path:', textLayer ? textLayer.style.clipPath : 'N/A');
        console.log('Text layer style webkit clip-path:', textLayer ? textLayer.style.webkitClipPath : 'N/A');
        ...
        // Check if text overlays are in the text layer
        const textOverlays = document.querySelectorAll('.text-overlay');
        console.log('Total text overlays:', textOverlays.length);
        console.log('Text overlays in text layer:', textLayer ? textLayer.querySelectorAll('.text-overlay').length : 0);
    };
    
    // Visual debug function to show the mask areas
    window.showMaskAreas = function() {
        console.log('=== SHOWING MASK AREAS ===');
        
        // Create a visual overlay to show where the mask areas are
        let debugOverlay = document.getElementById('mask-debug-overlay');
        if (debugOverlay) {
            debugOverlay.remove();
        }
        ...
        allTiles.forEach(tile => {
            ...
            if (!isEmpty && imgSrc.trim() !== '') {
                ...
                // Create a green rectangle to show this area
        ...
        console.log(`Showing ${occupiedCount} occupied tile areas in green`);
        console.log('Call showMaskAreas(false) to hide');
        
        // Store the hide function
        window.hideMaskAreas = function() {
            if (debugOverlay) {
                debugOverlay.remove();
            }
        };
