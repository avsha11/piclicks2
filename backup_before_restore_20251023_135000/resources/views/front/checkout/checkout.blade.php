...
@endsection
@push('js')
    @if(env('GOOGLE_API_KEY') && env('GOOGLE_API_KEY') !== 'DEMO_KEY')
    <script src="https://maps.googleapis.com/maps/api/js?key={{ env('GOOGLE_API_KEY') }}&libraries=places&loading=async&callback=initAutocomplete"></script>
    @else
    <script>
        // Google Maps not configured - show message instead
        function initAutocomplete() {
            const addressInput = document.getElementById("shopping_address");
            if (addressInput) {
                addressInput.placeholder = "Google Maps not configured - enter address manually";
                addressInput.readOnly = false;
            }
        }
        // Call immediately since we're not using async loading
        initAutocomplete();
    </script>
    @endif
    <script>
        ...
        function initAutocomplete() {
            ...
            if (typeof google !== 'undefined' && google.maps && google.maps.places) {
                // TODO: Migrate to PlaceAutocompleteElement when ready (deprecated API warning)
            ...
            autocomplete.addListener("place_changed", function() {
                ...
                // Google Maps not available - just enable manual input
                console.log("Google Maps not available - manual address entry only");
            }
        }
