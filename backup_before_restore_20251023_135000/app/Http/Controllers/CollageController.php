...
class CollageController extends Controller
{
    ...
    {
        try {
            Log::info('=== PREVIEW PAGE REQUEST START ===', ['unique_id' => $request->unique_id]);
            
            $unique_id = $request->unique_id;
            $designCollagePreviewData = $this->DesignCollageRepository->getOneMaster(['unique_id' => $unique_id, 'status' => 0]);
            $images = $this->DesignCollageRepository->getByWhere(['unique_id' => $unique_id, 'is_deleted' => 0], ['seq' => 'asc'])->toArray();
            
            // Convert model to array for easier access
            if ($designCollagePreviewData) {
                $designCollagePreviewData = $designCollagePreviewData->toArray();
            }
            
            Log::info('Preview data check', [
                'has_design_data' => !empty($designCollagePreviewData),
                'has_images' => !empty($images),
                'image_count' => count($images),
                'design_data' => $designCollagePreviewData ? array_keys($designCollagePreviewData) : 'null'
            ]);
            
            if (empty($designCollagePreviewData) || empty($images)) {
                Log::warning('Preview data missing', ['unique_id' => $unique_id]);
                return redirect()
                    ->route('front.upload-photos')
                    ->with(['images_empty_error' => 'Collage not found, try again.']);
            }
            ...
            Log::info('=== OPTIMIZED IMAGE PROCESSING ===');
            ...
            // Check if cached images already exist
            $cachedImage1 = 'temp/collage_merged_' . $unique_id . '-1.png';
            $cachedImage2 = 'temp/collage_merged_' . $unique_id . '-2.jpg';
            $cachedPath1 = storage_path('app/public/') . $cachedImage1;
            $cachedPath2 = storage_path('app/public/') . $cachedImage2;
            
            // Use cached images if they exist and are recent (less than 1 hour old)
            if (file_exists($cachedPath1) && file_exists($cachedPath2) && 
                (time() - filemtime($cachedPath1)) < 3600) {
                Log::info('Using cached preview images', [
                    'image1' => $cachedImage1,
                    'image2' => $cachedImage2,
                    'age_minutes' => round((time() - filemtime($cachedPath1)) / 60)
                ]);
                
                $mergedImage1 = asset('storage/' . $cachedImage1);
                $mergedImage2 = asset('storage/' . $cachedImage2);
            } else {
                Log::info('Generating new preview images');
                ...
                if (!file_exists($backgroundPath)) {
                    Log::error('Background image missing', ['path' => $backgroundPath]);
                    throw new \Exception('Background preview image not found at: ' . $backgroundPath);
                }
                
                if (!file_exists($collagePath)) {
                    Log::error('Collage image missing', ['path' => $collagePath]);
                ...
                // Calculate scaling and position (optimized)
                $bg_w = 1611;
                $bg_h = 1074;
                ...
                // Simplified scaling for faster processing
                $scale = min($avail_w / $img2_w, $avail_h / $img2_h, 1) * 0.8; // 80% of max size for faster processing
                ...
                // Save as PNG with lower quality for faster processing
                $mergedPath1 = storage_path('app/public/') . $cachedImage1;
                imagepng($preview_1, $mergedPath1, 6); // Reduced quality from 9 to 6
                ...
                // Generate second preview image (kitchen background) - simplified
                $preview_2 = @imagecreatefromjpeg(public_path('/assets/images/preview_kitchen.png'));
                if (!$preview_2) {
                    $preview_2 = @imagecreatefrompng(public_path('/assets/images/preview_kitchen.png'));
                }

                if ($preview_2) {
                    // Reload the collage image for the second preview
                    $img2 = @imagecreatefromjpeg($collagePath);
                    if (!$img2) {
                        $img2 = @imagecreatefrompng($collagePath);
                    }
                    ...
                    if ($img2) {
                        ...
                        // Use same scaling as first image for consistency
                        $img2_w = imagesx($img2);
                        $img2_h = imagesy($img2);
                        $scale = min($avail_w / $img2_w, $avail_h / $img2_h, 1) * 0.8;
                        $final_w = intval($img2_w * $scale);
                        $final_h = intval($img2_h * $scale);

                        // Position for second image (bottom)
                        $margin_bottom = intval($bg_h * 0.08);
                        $dst_x = $margin_left + intval(($avail_w - $final_w) / 2);
                        ...
                        // Save merged image with lower quality for faster processing
                        $mergedPath2 = storage_path('app/public/') . $cachedImage2;
                        imagejpeg($preview_2, $mergedPath2, 60); // Reduced quality from 80 to 60
            ...
            Log::info('Image path verification', [
                ...
                'collage_exists' => file_exists($collagePath),
                'image_path_from_db' => $designCollagePreviewData['image_path'] ?? 'null'
            ]);
            ...
            */
            ...
            if (!empty($designCollagePreviewData['filter']) && $designCollagePreviewData['filter'] !== 'filter-original') {
                Log::info("Applying filter to preview image", ['filter' => $designCollagePreviewData['filter']]);
                $this->applyFilterToImage($img2, $designCollagePreviewData['filter']);
            } else {
                Log::info("No filter applied", ['reason' => empty($designCollagePreviewData['filter']) ? 'empty' : 'original']);
            }
