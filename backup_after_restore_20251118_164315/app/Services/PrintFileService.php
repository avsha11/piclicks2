...
class PrintFileService
{
    ...
    private function renderImage($canvas, string $imagePath, int $blockW, int $blockH, string $zoom, string $rotate, array $frameConfig = [], bool $visibleAreaOnly = false, ?float $editorBlockW = null, ?float $editorBlockH = null): void
    {
        ...
        // Editor calculates: scaleMin = max(viewportWidth/imgWidth, viewportHeight/imgHeight)
        // The stored minZoom IS this scaleMin value
        // Then applies zoom: finalScale = scaleMin * (currentZoom / minZoom)
        // For preview, we need to match this calculation exactly
        
        // If we have stored minZoom, we can work backwards to find the viewport that was used
        // Editor uses different viewport logic for single tiles (91x80) vs stretched tiles
        if ($visibleAreaOnly && $minZoom !== null && $minZoom > 0.0 && $editorBlockW !== null && $editorBlockH !== null && $editorBlockW > 0 && $editorBlockH > 0) {
            $viewportW = null;
            $viewportH = null;
            
            // Check if this is a single-tile block (91x80 in editor pixels)
            // Editor uses fixed viewport sizes for single tiles: 342x300, 456x400, or 570x500
            if (abs($editorBlockW - 91.0) < 1.0 && abs($editorBlockH - 80.0) < 1.0) {
                // Single tile - try each fixed viewport and pick the one that matches minZoom best
                $fixedViewports = [
                    [342.0, 300.0],
                    [456.0, 400.0],
                    [570.0, 500.0]
            ...
            // If not single tile or no match found, calculate for stretched tiles
            if ($viewportW === null || $viewportH === null) {
                ...
                $viewportW = min($editorBlockW * 2.0, $maxViewportW);
                $viewportH = min($editorBlockH * 2.0, $maxViewportH);
            ...
            // Editor: scaleMin = max(viewportW/srcW, viewportH/srcH) = minZoom
            // Editor applies: finalScale = scaleMin * (currentZoom / minZoom)
            // The image is scaled by finalScale and positioned at top-left in the viewport
            
            // Calculate the image size as it appears in the editor's viewport
            // This matches exactly what the editor shows
            $editorFinalScale = $minZoom * $zoomScaleX; // This is currentZoom
            $editorScaledW = $srcW * $editorFinalScale;
            $editorScaledH = $srcH * $editorFinalScale;
            
            // Now we need to scale from the editor's viewport to the preview's target area
            // The target area might be smaller (if frame exists) or same size (if no frame)
            // We scale proportionally to maintain the same relative crop/position
            $scaleFromViewportX = $targetW / $viewportW;
            $scaleFromViewportY = $targetH / $viewportH;
            
            // Use uniform scaling to maintain aspect ratio and relative crop
            // This ensures the same portion of the image is visible
            $scaleFromViewport = min($scaleFromViewportX, $scaleFromViewportY);
            
            // Apply scaling to get final dimensions
            $scaledW = max(1, (int) round($editorScaledW * $scaleFromViewport));
            $scaledH = max(1, (int) round($editorScaledH * $scaleFromViewport));
            
            // Log for debugging
            Log::info("Preview zoom calculation", [
                'src' => "{$srcW}x{$srcH}",
                'target' => "{$targetW}x{$targetH}",
                'viewport' => "{$viewportW}x{$viewportH}",
                'editorBlock' => "{$editorBlockW}x{$editorBlockH}",
                'minZoom' => $minZoom,
                'currentZoom' => $currentZoom,
                'zoomRatio' => $zoomScaleX,
                'editorFinalScale' => $editorFinalScale,
                'editorScaled' => "{$editorScaledW}x{$editorScaledH}",
                'scaleFromViewport' => $scaleFromViewport,
                'scaled' => "{$scaledW}x{$scaledH}",
                'frameInset' => $frameInset,
            ]);
        } else {
            // Standard cover fit calculation (for print or when zoom not available)
