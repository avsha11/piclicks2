...
class PreviewRenderer
{
    ...
    private function renderTextOverlays($canvas, $maskCanvas, int $canvasWidth, int $canvasHeight, array $master, int $minCol, int $minRow, int $tileWidth, int $tileHeight, int $gap): void
    {
        ...
        foreach ($overlays as $overlay) {
            ...
            // Only need to subtract the occupied tile offset to account for minCol/minRow
            // The text layer in the editor is positioned to align with the grid, so positions
            // are already relative to the grid's coordinate system (no padding offset needed)
            ...
            // CSS uses transform: translate(-50%, -50%) to center based on the element's bounding box
            // The key insight: we need to use a CONSISTENT bounding box calculation across all fonts
            // The editor centers based on the element's actual bounding box (getBoundingClientRect)
            // which includes the text content with minimal padding, centered at the pivot point
            
            // Calculate the center position (where the editor's pivot point is)
            $centerX = $x + $translateX + (self::TEXT_ADJUST_DX * $scaleFactor);
            $centerY = $y + $translateY + (self::TEXT_ADJUST_DY * $scaleFactor);

            // For consistent positioning across fonts, we need to:
            // 1. Use the bounding box center (which varies by font)
            // 2. But adjust to account for the font's baseline position
            // The editor's getBoundingClientRect() returns a box that's centered on the text content
            // We need to match that behavior by using the bounding box center, not the baseline center
            
            // Calculate the bounding box center relative to the baseline
            // The bounding box center in bbox coordinates is:
            $bboxCenterX = ($minX + $maxX) / 2;
            $bboxCenterY = ($minY + $maxY) / 2;
            
            // To position the bounding box center at (centerX, centerY):
            // The baseline X position should be: centerX - bboxCenterX
            // The baseline Y position should be: centerY - bboxCenterY (since baseline is at y=0 in bbox coords)
