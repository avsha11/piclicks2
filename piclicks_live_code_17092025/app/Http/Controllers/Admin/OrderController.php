...
class OrderController extends Controller
{
    ...
    public function orderDetails(Request $request)
    {
        try {
            ...
            // Generate preview PNG images for collage orders using PreviewRenderer
            if (isset($order->orderdetail_data) && count($order->orderdetail_data) > 0) {
                $previewRenderer = new PreviewRenderer();
                
                foreach ($order->orderdetail_data as $orderData) {
                    if (($orderData->name == 'collage' || $orderData->name == 'artgallery') && isset($orderData->design_collage_master)) {
                        try {
                            $uniqueId = $orderData->design_collage_master->unique_id ?? $orderData->collage_unique_id;
                            
                            if ($uniqueId) {
                                // Get master data
                                $masterData = $this->DesignCollageRepository->getOneMaster(['unique_id' => $uniqueId]);
                                
                                if ($masterData) {
                                    // Get tiles
                                    $tilesCollection = $this->DesignCollageRepository->getByWhere([
                                        'unique_id' => $uniqueId,
                                        'is_deleted' => 0
                                    ], ['seq' => 'asc']);
                                    ...
                                    if ($tilesCollection && $tilesCollection->count() > 0) {
                                        ...
                                        if (file_exists($previewFullPath)) {
                                            ...
                                            $orderData->preview_image_path = $previewPath;
                                        } else {
                                            // If preview doesn't exist, generate it (same as Preview page)
                                            ...
                                            if ($previewResult && isset($previewResult['path'])) {
                                                // Add preview path to orderData for use in view
                                                $orderData->preview_image_path = $previewResult['path'];
                                            } else {
                                                // Fallback to original image_path if preview generation fails
                                                $orderData->preview_image_path = $orderData->design_collage_master->image_path ?? null;
                                            }
                                        }
                                    } else {
                                        // No tiles found, use fallback
                                        $orderData->preview_image_path = $orderData->design_collage_master->image_path ?? null;
                                    }
                                } else {
                                    // No master data, use fallback
                                    $orderData->preview_image_path = $orderData->design_collage_master->image_path ?? null;
                                }
                            } else {
                                // No unique_id, use fallback
                                $orderData->preview_image_path = $orderData->design_collage_master->image_path ?? null;
                            }
                        } catch (Exception $e) {
                            Log::warning('Error generating preview image for order detail', [
                                'order_detail_id' => $orderData->id ?? null,
                                'error' => $e->getMessage()
                            ]);
                            // Fallback to original image_path on error
                            $orderData->preview_image_path = $orderData->design_collage_master->image_path ?? null;
                        }
                    }
                }
